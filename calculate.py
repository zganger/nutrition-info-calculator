...
def calculate(recipe_info: Dict, portions: int):
    ...
    for ingredient_name, ingredient_info in recipe_info.items():
        print(f"The following accounts for nutrition info from {ingredient_name} per portion:")
...
def start():
    ...
    while entering_ingredients:
        ...
        ingredient_info = {
            "nutrition": {},
            "servings": int(input(f"How many servings of {ingredient_name} are you using?\n"))
        }
        for attr in {"calories"}:
            ingredient_info["nutrition"][attr] = int(input(f"How many {attr} per serving of {ingredient_name}?\n"))

        recipe_info[ingredient_name] = ingredient_info

        entering_ingredients = (input("Are there any more ingredients to enter? [Y/n]\n") or "Y").lower() == 'y'
